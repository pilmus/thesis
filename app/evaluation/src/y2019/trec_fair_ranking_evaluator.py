...
def evaluate(qseq_file, gt_file, group_annot_file, group_definition, run_files_prefix, run_files):
    task = FairRankingTask(qseq_file, gt_file, group_annot_file)
    ...
    for run in run_files:
        with open(os.path.join(os.path.dirname(run_files_prefix), 'eval_results', group_definition,
                               f"{os.path.splitext(os.path.basename(run))[0]}.csv"), 'w') as f_out:
            f_out.write(
        ...
        # fig = plt.figure()
        # plt.savefig(os.path.dirname(run_files_prefix), 'plots', group_definition, 'performance-all.pdf')
        # plt.show()
...
def main():
    ...
    parser.add_argument('--run_files_prefix', help='location of the run files')
    parser.add_argument('--run_files', nargs='*', help='runfiles to evaluate', required=True)
    args = parser.parse_args()

    qseq_file = args.query_sequence_file
    gt_file = args.groundtruth_file
    group_annot_file = args.group_annotations_file
    group_definition = args.group_definition

    # run_files_prefix = 'fairRuns'

    run_files_prefix = args.run_files_prefix
    # run_files = [
    #     'snerkle.json'
    # ]
    run_files = args.run_files

    evaluate(qseq_file, gt_file, group_annot_file, group_definition, run_files_prefix, run_files)
