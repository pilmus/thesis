...
def select(query_wise_rankings_directory, ranking_scores_directory, k, balancing_factor):
    ...
    feature_numbers = list(map(lambda file_path: int(os.path.basename(file_path).replace("F", "").replace(".txt", "")),
                               ranking_score_files))

    query_wise_rankings_files = glob.glob(os.path.join(query_wise_rankings_directory, "*"))
    query_ids = list(
        set(map(lambda file_path: int(os.path.basename(file_path).split("q")[1]), query_wise_rankings_files)))
    ...
    for cnt in tqdm(range(k)):
        feat_subset.append(max_feature)
        FkDocs = find_doclist(max_feature, FkDocs, query_wise_rankings_directory, query_ids)
        max_feature, fscores = upd_rel_score(FkDocs, fscores, feat_subset, ranking_scores_directory,
                                             query_wise_rankings_directory, feature_numbers, query_ids,
                                             balancing_factor)
        # feat_subset.append(max_feature)
...
def upd_rel_score(feat_subset, scores, feat_subset1, ranking_scores_directory, query_wise_rankings_directory,
                  feature_numbers, query_ids, balancing_factor):
    ...
    for j in feature_numbers:
        ...
        if scores[j - 1] != float("-inf"):
            ...
            scores[j - 1] = scores[j - 1] - (
                        (balancing_factor * fvar[j - 1]) + (2 * balancing_factor * math.sqrt(fvar[j - 1]) * Sim_sum))
