...
def pluck_feature_and_generate_ranking_file(input_example_file, target_directory, feature_number):
    ...
    os.system("paste -d' ' query_ids Q0 document_ids rank feature_values label | sort -k1,1n -k5,5gr > {}".format(
        output_file_path))
    os.remove("query_ids")
    os.remove("Q0")
...
for feature_number in range(1, total_number_of_features + 1):
    ...
    pluck_feature_and_generate_ranking_file(training_file, target_directory, feature_number)
