...
```
where `RUNFILE` is a json run file, `QRELS` is a json qrels file, and `GROUPFILE` is a comma-separated file with 
group labels.  Each line of the group file has to contain two fields, the first is the document id and the second is 
the integer document-level group label.  